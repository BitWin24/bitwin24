...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 MAG.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzMAGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMAGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMAGReset.connect(boost::bind(NotifyzMAGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMAGReset.disconnect(boost::bind(NotifyzMAGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmagTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
