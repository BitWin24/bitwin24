...
/**
 * Fees smaller than this (in ubwi) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_3000);
...
{
    return zbwiTracker->HasSerial(bnSerial);
}
                           ...
                           const isminefilter& filter) const
{
    ...
    listSent.clear();
    strSentAccount = strFromAccount;
    bool wildcardAccount = (targetAccount == string("*"));
    const auto isTargetAccount = [&](const std::string& acc) -> bool {
        return wildcardAccount || acc == targetAccount;
    };
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZBWI = GetBoolArg("-zapwallettxes", false);
    if (fCheckZBWI)
        zbwiTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zbwi
            if (fCheckZBWI && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
void CWallet::TxAddedToWallet(const CWalletTx& wtxIn)
{
    ...
    if (nDepth != 0 || wtxIn.InMempool()) {
        ...
        for (size_t i = 0; i < wtxIn.vout.size(); i++) {
            const auto& output = wtxIn.vout[i];
    ...
    // vector<COutput> vCoins1;
    // AvailableCoinsNew(vCoins1);

    // LogPrintf("Dump vCoins:\n");
    // for (const auto& out: vCoins) {
    //     LogPrintf("%s - %d\n", out.tx->GetHash().ToString(), out.i);
    // }
    // LogPrintf("Dump vCoins1:\n");
...
BalanceInfo CWallet::RecalculateBalanceInfo()
{
    ...
    size_t oldTxs = 0;
    const auto time = GetAdjustedTime();
    for (auto it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        const auto& wtxIn = it->second;
        UpdateBalanceOnAddedTransaction(tmp, wtxIn);
        if (wtxIn.GetTxTime() >= time - SECONDS_PER_DAY) {
            balinfo.tmpTxs.insert(make_pair(wtxIn.GetHash(), wtxIn));
            continue;
        }

        oldTxs++;
        UpdateBalanceOnAddedTransaction(balinfo, wtxIn);
    }
    balanceInfo = std::move(balinfo);

    LogPrintf("RecalculateBalanceInfo() : stable available=%d, total available=%d, oldTxs=%d, tmpTxs.size()=%d\n",
        balanceInfo.nTotal, tmp.nTotal, oldTxs, balanceInfo.tmpTxs.size());
    return balanceInfo;
}
...
BalanceInfo CWallet::GetBalanceInfo()
{
    ...
    auto ret = balanceInfo.CopyBalances();
    const auto time = GetAdjustedTime();
    const auto tmpTxsSize = balanceInfo.tmpTxs.size();
    for (auto it = balanceInfo.tmpTxs.begin(); it != balanceInfo.tmpTxs.end(); ) {
        UpdateBalanceOnAddedTransaction(ret, it->second);

        if (it->second.GetTxTime() < time - SECONDS_PER_DAY) {
            UpdateBalanceOnAddedTransaction(balanceInfo, it->second);
            it = balanceInfo.tmpTxs.erase(it);
        }
        else {
            ++it;
        }
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zbwiTracker->GetMints(true);
    ...
    return zbwiTracker->GetBalance(false, false);
}
...
{
    return zbwiTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zbwiTracker->ListMints(true, true, true);
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (IsLockedCoin((*it).first, i) && nCoinType != ONLY_3000)
                    continue;
        ...
        const auto t_end = boost::chrono::high_resolution_clock::now();
        LogPrintf("TIME: AvailableCoinsNew (%d): %d\n", nCoinType,
            boost::chrono::duration_cast<boost::chrono::milliseconds>(t_end - t_begin).count());
    }
}
...

    {
        ...
        for (auto it = unspents.begin(); it != unspents.end(); it++) {
            ...
            if (nCoinType == STAKABLE_COINS) {
                ...
                CTxDestination txAddress;
                if ( ExtractDestination(output.scriptPubKey, txAddress) && !IsStakingEnabled( CBitcoinAddress(txAddress) ) ) {
                    continue;
                }
            }
...
bool CWallet::SelectStakeCoins(std::vector<std::unique_ptr<CStakeInput>>& listInputs, CAmount nTargetAmount)
{
    ...
    if(Params().ZeroCoinEnabled()) {
        if (GetBoolArg("-zbwistake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
            //Only update zBWI set once per update interval
            bool fUpdate = false;
            ...
            set<CMintMeta> setMints = zbwiTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZbitwin24Balance = GetZerocoinBalance(false);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 BITWIN24
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 BITWIN24 over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 BITWIN24+1 ( bit on if present )
            // bit 1 - 1000 BITWIN24+1
            // bit 2 - 100 BITWIN24+1
            // bit 3 - 10 BITWIN24+1
            // bit 4 - 1 BITWIN24+1
            // bit 5 - .1 BITWIN24+1
            ...
            if (fFound10000 && fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1) { //if fulfilled
                ...
                if ((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue == ((3000 * COIN) + 10000000)) {
                    fAccepted = true;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nObfuscationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT3000IFMN : ONLY_DENOMINATED);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 BITWIN24 collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 BITWIN24 collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    ...
                    } else if (coin_type == ONLY_NOT3000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 BITWIN24.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT3000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 BITWIN24.");
                    } else {
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            ...
            bool fAccepted = false;
            if ((obfuScationPool.sessionDenom & (1 << 0)) && v == ((3000 * COIN) + 10000000)) {
                fAccepted = true;
...
void CWallet::AutoZeromint()
{
    ...
    // zBWI are integers > 0, so we can't mint 10% of 9 BITWIN24
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zBWI\n", nBalance);
        return;
    }

    // Percentage of zBWI we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zBWI (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zBWI amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zBWI amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zBWI We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough BITWIN24 to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zBWI. Current percentage of zBWI: %lf%%\n",
...
bool CWallet::CreateZBWIOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZBWI(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZBWIOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zbwi output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to BitWin24
    ...
    // Sign if these are bitwin24 outputs - NOTE that zBWI outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZBWI_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZBWI_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZBWI_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zBWI privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZBWI_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a BITWIN24 transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZBWI_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), ZBWI_BAD_SERIALIZATION);
            return false;
        }

        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zBWI
            receipt.SetStatus(_("The coin spend has been used"), ZBWI_SPENT_USED_ZBWI);

            uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
            if (!zbwiTracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zbwiTracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zbwiTracker->UpdateState(meta))
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZBWI_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), ZBWI_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZBWI_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zBWI"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZBWI_TRX_CREATE;
    ...
    if (vSelectedMints.empty()) {
        setMints = zbwiTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zBWI mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZBWI_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zBWI require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZBWI_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZBWI_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zbwiTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zbwiTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zbwiTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zbwiTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzbwiAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzbwiAuto ? "-autozbwibackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZBWIBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozbwibackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozbwibackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozbwibackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zbwibackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zbwibackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZBWI_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZBWI_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZBWI_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zbwiTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZBWI_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZBWI_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zbwiTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zbwiTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZBWI_SPEND_OKAY);  // When we reach this point spending zBWI was successful
...
{
    CMintMeta meta;
    if (!zbwiTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zbwiTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zbwiTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zbwiTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zbwiTracker->UpdateState(meta);
    } else {
